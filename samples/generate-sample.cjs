...
let running = 0;
const records = trades.map((t, i) => {
  running = round2(running + t.pl);
  const entrySecs = t.eh * 3600 + t.em * 60;
  const exitSecs = t.xh * 3600 + t.xm * 60;
  return {
    rowNumber: i + 2,
    date: `${t.y}-${pad(t.mo + 1)}-${pad(t.d)}`,
    entryTime: entrySecs,
    exitTime: exitSecs,
    tradeNumber: t.no,
    duration: exitSecs - entrySecs,
    direction: t.dir,
    symbol: t.sym,
    entryPrice: t.entry,
    exitPrice: t.exit,
...
const rows = records.map((r) => {
  ...
  return [
    serial(yy, mm - 1, dd),
    r.entryTime / 86400, r.exitTime / 86400, r.tradeNumber, r.duration / 86400,
    r.direction, r.symbol, r.entryPrice, r.exitPrice, r.size, r.profitLoss,
    r.setup, r.reasonEmotion, r.runningPnl, r.note,
  ];
});
...
// Also emit a typed module so the app can offer an instant, network-free demo.
const ts =
  '// AUTO-GENERATED by samples/generate-sample.cjs — do not edit by hand.\n' +
  "import type { TradeRecord } from '../types';\n\n" +
  'export const sampleTrades: TradeRecord[] = ' + JSON.stringify(records) + ';\n';
const tsOut = path.join(__dirname, '..', 'src', 'data', 'sampleTrades.ts');
fs.mkdirSync(path.dirname(tsOut), { recursive: true });
fs.writeFileSync(tsOut, ts);

console.log('Wrote', out, 'and', tsOut, 'with', records.length, 'sample trades across', YEAR);
